class Api::StatsController < Api::AuthenticatedController
  ...
  def browsers_doughnut
    render json: doughnut(collection, :browser, duration, &helper.browsers_labels)
  end

  def browsers_line
    render json: line(collection, :browser, duration, &helper.browsers_labels(true))
  end

  def providers_doughnut
    data = doughnut collection, :provider, duration, Proc.new { |data|
  ...
  def providers_line
    data = line collection, :provider, duration, &helper.providers_labels(true)
  ...
  def categories_doughnut
    render json: doughnut(collection, :category_id, duration, &helper.categories_labels)
  end

  def categories_line
    render json: line(collection, :category_id, duration, &helper.categories_labels(true))
  end
  
  def channels_doughnut
    render json: doughnut(collection, helper.channel_column, duration,
                          helper.limit_data(duration), &helper.channels_labels)
  end

  def channels_line
    render json: line(collection, helper.channel_column, duration, &helper.channels_labels(true))
  end

  def series_doughnut
    render json: find_series(collection, :title, duration, &make_series_doughnut)
  end

  def series_line
    render json: find_series(collection, :title, duration, &make_series_line)
  end

  private

  def duration
    return params[:duration] == "1" if params[:duration]
    false
  end
  ...
  def doughnut(collection, column, duration = false, adjust_data = nil)
    column_data = collection.group(column)
    column_data = duration ? column_data.sum(:duration) : column_data.count
    data = column_data.map do |record, count|
      { label: record, value: (duration ? helper.seconds_to_hours(count) : count) }
    end
  ...
  def line(collection, column, duration = false, adjust_data = nil)
    column_data = collection.group(column).order("videos.created_at").
        group(helper.month_column)
    column_data = duration ? column_data.sum(:duration) : column_data.count
    ...
    column_data.each do |key, value|
      ...
      datasets[record][month] += (duration ? helper.seconds_to_hours(value) : value)
    end
